...
export function Base64Token({ symbol, onImageGenerated, aToken }: { symbol: string; aToken?: boolean; onImageGenerated: (base64: string) => void }) {
  ...
  useEffect(() => {
    if (!loading && ref.current && ref.current?.contentDocument) {
      if (aToken) {
        ...
        onImageGenerated(`data:image/svg+xml;base64,${window.btoa(unescape(encodeURIComponent(s)))}`)
      } else {
  ...
  return (
    ...
    >
      <object style={{ opacity: 1 }} ref={ref} id="svg" data={`/icons/tokens/${symbol.toLowerCase()}.svg`} onLoad={() => setLoading(false)} />
...
export const ATokenIcon = forwardRef<SVGSVGElement, ATokenIconProps>(({ symbol }, ref) => (
  ...
  >
    ...
    <g id="Group_29109">
      ...
      />
      {symbol && <image x="25" y="25" href={`/icons/tokens/${symbol.toLowerCase()}.svg`} width="206" height="206" />}
    </g>
  </svg>
))
...
function SingleTokenIcon({ symbol, aToken, ...rest }: TokenIconProps) {
  return (
    <Icon {...rest} sx={{ display: 'flex', position: 'relative', borderRadius: '50%', ...rest.sx }}>
      {aToken ? <ATokenIcon symbol={symbol} /> : <img src={imageMap.token[symbol.toLowerCase() as keyof typeof imageMap.token]} width="100%" height="100%" alt={`${symbol} icon`} />}
    </Icon>
  )
}
    ...
    return (
      <Box sx={{ display: 'inline-flex', position: 'relative' }}>
        {symbols.map((symbol, ix) => (
          <SingleTokenIcon {...rest} key={symbol} symbol={symbol} sx={{ ml: ix === 0 ? 0 : `calc(-1 * 0.5em)`, ...rest.sx }} />
        ))}
      </Box>
    )
  return (
    <Badge badgeContent={<SingleTokenIcon symbol={badgeSymbol} sx={{ border: '1px solid #fff' }} fontSize="small" />} sx={{ '.MuiBadge-anchorOriginTopRight': { top: 9 } }}>
      {symbols.map((symbol, ix) => (
        <SingleTokenIcon {...rest} key={symbol} symbol={symbol} sx={{ ml: ix === 0 ? 0 : 'calc(-1 * 0.5em)', ...rest.sx }} />
      ))}
    </Badge>
