...
import { RenderElementProps, RenderLeafProps, useFocused, useSelected, useSlate } from 'slate-react'

import { Box } from '@mui/material'
import * as css from '../../styles/CustomHtml.css'
import { CommandElement, EmoticonElement, LinkElement, MentionElement, TemplateElement } from './slate'
...
function RenderTemplateElement({ attributes, element, children }: { element: TemplateElement } & RenderElementProps) {
  ...
  return (
    <Box
      ...
      sx={{
        ...
        padding: '0 2px',
        borderRadius: '4px',
        backgroundColor: '#4685FF33',
        boxShadow: selected ? '0 0 0 var(--_164xfge25) var(--oq6d07e)' : 'none',
      }}
...
function RenderMentionElement({ attributes, element, children }: { element: MentionElement } & RenderElementProps) {
  const selected = useSelected()
...
function RenderCommandElement({ attributes, element, children }: { element: CommandElement } & RenderElementProps) {
  const selected = useSelected()
  const focused = useFocused()
...
function RenderEmoticonElement({ attributes, element, children }: { element: EmoticonElement } & RenderElementProps) {
  ...
  return (
    <span className={css.EmoticonBase} {...attributes}>
      <span
        ...
        contentEditable={false}
      >
        {element.key.startsWith('mxc://') ? <img className={css.EmoticonImg} src={mx.mxcUrlToHttp(element.key) ?? element.key} alt={element.shortcode} /> : element.key}
        {children}
      </span>
    </span>
  )
...
function RenderLinkElement({ attributes, element, children }: { element: LinkElement } & RenderElementProps) {
  return (
...
export function RenderElement({ attributes, element, children }: RenderElementProps) {
  switch (element.type) {
    case BlockType.Template:
      return (
        <RenderTemplateElement attributes={attributes} element={element}>
          {children}
