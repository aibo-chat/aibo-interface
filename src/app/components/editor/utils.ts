...
import { CommandElement, EmoticonElement, FormattedText, HeadingLevel, LinkElement, MentionElement, TemplateElement } from './slate'

const ALL_MARK_TYPE: MarkType[] = [MarkType.Bold, MarkType.Code, MarkType.Italic, MarkType.Spoiler, MarkType.StrikeThrough, MarkType.Underline]
...
const NESTED_BLOCK = [BlockType.OrderedList, BlockType.UnorderedList, BlockType.BlockQuote, BlockType.CodeBlock]
...
export const createTemplateElement = (content: string): TemplateElement => ({
  type: BlockType.Template,
  content,
  children: [{ text: '' }],
})
export const createMentionElement = (id: string, name: string, highlight: boolean): MentionElement => ({
...
export const createLinkElement = (href: string, children: string | FormattedText[]): LinkElement => ({
  type: BlockType.Link,
  href,
...
export const getPointUntilChar = (editor: Editor, cursorPoint: BasePoint, options: PointUntilCharOptions): BasePoint | undefined => {
...
export const getBeginCommand = (editor: Editor): string | undefined => {
  ...
  if (Element.isElement(secondInline) && secondInline.type === BlockType.Command) return secondInline.command
  return undefined
}
