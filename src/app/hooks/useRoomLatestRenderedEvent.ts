...
export const useRoomLatestRenderedEvent = (room: Room, type?: Array<string>) => {
  ...
  useEffect(() => {
    const getLatestEvent = (): MatrixEvent | undefined => {
      const liveEvents = room
        .getLiveTimeline()
        .getEvents()
        .filter((evt) => (type ? type.includes(evt.getType()) : true))
  ...
  }, [room, hideMembershipEvents, hideNickAvatarEvents, showHiddenEvents, type])
