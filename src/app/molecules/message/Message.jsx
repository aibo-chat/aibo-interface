...
import { getUsername, getUsernameOfRoomMember, parseReply, trimHTMLReply } from '../../../util/matrixUtil'
import colorMXID from '../../../util/colorMXID'
import { getEventCords } from '../../../util/common'
import { redactEvent, sendReaction } from '../../../client/action/roomTimeline'
import { openEmojiBoard, openProfileViewer, openReadReceipts, openViewSource, replyTo } from '../../../client/action/navigation'
...
const MessageHeader = React.memo(({ userId, username, timestamp, fullTime }) => (
  <div className="message__header">
    <Text style={{ color: colorMXID(userId) }} className="message__profile" variant="b1" weight="medium" span>
...
const MessageReplyWrapper = React.memo(({ roomTimeline, eventId }) => {
  ...
  return (
    <div className="message__reply-wrapper" onClick={focusReply} onKeyDown={focusReply} role="button" tabIndex="0">
...
const MessageBody = React.memo(({ senderName, body, isCustomHTML, isEdited, msgType }) => {
  ...
  if (isCustomHTML) {
    try {
      content = twemojify(sanitizeCustomHtml(initMatrix.matrixClient, body), undefined, true, false, true)
    } catch {
  ...
  if (content.type === 'img') {
    ...
    if (nEmojis <= 10 && content.every((element) => (typeof element === 'object' && element.type === 'img') || (typeof element === 'string' && /^[\s\ufe0f]*$/g.test(element)))) {
      emojiOnly = true
    }
  }
...
function MessageEdit({ body, onSave, onCancel }) {
  ...
  return (
    ...
    >
      <Input forwardRef={editInputRef} onKeyDown={handleKeyDown} value={body} placeholder="Edit message" required resizable autoFocus />
    ...
    <>
      {userIds.map((userId, index) => (
        <React.Fragment key={userId}>
          ...
          {index < userIds.length - 1 && <span style={{ opacity: '.6' }}>{index === userIds.length - 2 ? ' and ' : ', '}</span>}
        </React.Fragment>
      ))}
...
function MessageReaction({ reaction, shortcode, count, users, isActive, onClick }) {
  ...
  return (
    <Tooltip className="msg__reaction-tooltip" content={<Text variant="b2">{users.length > 0 ? genReactionMsg(users, reaction, shortcode) : 'Unable to load who has reacted'}</Text>}>
      <button onClick={onClick} type="button" className={`msg__reaction${isActive ? ' msg__reaction--active' : ''}`}>
        {customEmojiUrl ? <img className="react-emoji" draggable="false" alt={shortcode ?? reaction} src={customEmojiUrl} /> : twemojify(reaction, { className: 'react-emoji' })}
...
function isMedia(mE) {
  return (
    mE.getContent()?.msgtype === 'm.file' || mE.getContent()?.msgtype === 'm.image' || mE.getContent()?.msgtype === 'm.audio' || mE.getContent()?.msgtype === 'm.video' || mE.getType() === 'm.sticker'
  )
}
...
const MessageOptions = React.memo(({ roomTimeline, mEvent, edit, reply }) => {
  ...
  return (
    <div className="message__options">
      {canSendReaction && <IconButton onClick={(e) => pickEmoji(e, roomId, mEvent.getId(), roomTimeline)} src={EmojiAddIC} size="extra-small" tooltip="Add reaction" />}
      <IconButton onClick={() => reply()} src={ReplyArrowIC} size="extra-small" tooltip="Reply" />
      {senderId === mx.getUserId() && !isMedia(mEvent) && <IconButton onClick={() => edit(true)} src={PencilIC} size="extra-small" tooltip="Edit" />}
      <ContextMenu
        content={() => (
          <>
            <MenuHeader>Options</MenuHeader>
            <MenuItem iconSrc={TickMarkIC} onClick={() => openReadReceipts(roomId, roomTimeline.getEventReaders(mEvent))}>
              Read receipts
            </MenuItem>
            ...
            {(canIRedact || senderId === mx.getUserId()) && (
              <>
                ...
                <MenuItem
                  ...
                  iconSrc={BinIC}
                  onClick={async () => {
                    const isConfirmed = await confirmDialog('Delete message', 'Are you sure that you want to delete this message?', 'Delete', 'danger')
        ...
        render={(toggleMenu) => <IconButton onClick={toggleMenu} src={VerticalMenuIC} size="extra-small" tooltip="Options" />}
      />
    </div>
  )
})
...
function genMediaContent(mE) {
  ...
  switch (msgType) {
    case 'm.file':
      return <Media.File name={mContent.body} link={mx.mxcUrlToHttp(mediaMXC)} type={mContent.info?.mimetype} file={mContent.file || null} />
    case 'm.image':
      return (
        <Media.Image
          name={mContent.body}
    ...
    case 'm.audio':
      return <Media.Audio name={mContent.body} link={mx.mxcUrlToHttp(mediaMXC)} type={mContent.info?.mimetype} file={mContent.file || null} />
    case 'm.video':
...
function Message({ mEvent, isBodyOnly, roomTimeline, focus, fullTime, isEdit, setEdit, cancelEdit }) {
  ...
  const haveReactions = roomTimeline ? reactionTimeline.has(eventId) || !!mEvent.getServerAggregatedRelation('m.annotation') : false
  ...
  return (
    <div className={className.join(' ')}>
      {isBodyOnly ? <div className="message__avatar-container" /> : <MessageAvatar roomId={roomId} avatarSrc={avatarSrc} userId={senderId} username={username} />}
      <div className="message__main-container">
        {!isBodyOnly && <MessageHeader userId={senderId} username={username} timestamp={mEvent.getTs()} fullTime={fullTime} />}
        {roomTimeline && isReply && <MessageReplyWrapper roomTimeline={roomTimeline} eventId={mEvent.replyEventId} />}
        {!isEdit && <MessageBody senderName={username} isCustomHTML={isCustomHTML} body={isMedia(mEvent) ? genMediaContent(mEvent) : customHTML ?? body} msgType={msgType} isEdited={isEdited} />}
        {isEdit && (
          <MessageEdit
            body={customHTML ? html(customHTML, { kind: 'edit', onlyPlain: true }).plain : plain(body, { kind: 'edit', onlyPlain: true }).plain}
        ...
        {roomTimeline && !isEdit && <MessageOptions roomTimeline={roomTimeline} mEvent={mEvent} edit={edit} reply={reply} />}
      </div>
    </div>
  )
}
