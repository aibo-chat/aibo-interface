...
import { MatrixClient, MatrixEvent, Room, RoomStateEvent } from 'matrix-js-sdk'
...
const permissionsInfo: {
  ...
  'm.room.condition': {
    ...
    description: 'Set minimum power level to change room/space conditions.',
    default: 100,
  },
  state_default: {
...
const roomPermsGroups = {
  ...
  // 'Room profile permissions': ['m.room.avatar', 'm.room.name', 'm.room.topic', DefedEventType.RoomConditions],
  'Room profile permissions': ['m.room.name', 'm.room.topic', DefedEventType.RoomConditions],
...
const spacePermsGroups = {
  ...
  'Manage members permissions': ['invite', 'kick', 'ban'],
  // 'Space profile permissions': ['m.room.avatar', 'm.room.name', 'm.room.topic', DefedEventType.RoomConditions],
  'Space profile permissions': ['m.room.name', 'm.room.topic', DefedEventType.RoomConditions],
  'Settings permissions': ['state_default', 'm.room.canonical_alias', 'm.room.power_levels'],
}

function useRoomStateUpdate(roomId: string) {
  const [, forceUpdate] = useForceUpdate()
  const mx = initMatrix.matrixClient as MatrixClient

  useEffect(() => {
    const handleStateEvent: (event: MatrixEvent) => void = (event) => {
...
interface IRoomPermissionsProps {
  roomId: string
}
const RoomPermissions: React.FC<IRoomPermissionsProps> = ({ roomId }) => {
  ...
  const canChangePermission = room.currentState.maySendStateEvent('m.room.power_levels', mx.getUserId() as string)
  const myPowerLevel = room.getMember(mx.getUserId() as string)?.powerLevel ?? 100

  const handlePowerSelector = (e: Event | React.MouseEvent<HTMLButtonElement, MouseEvent>, permKey: string, parentKey: string | number | undefined, powerLevel: number) => {
    ...
    openReusableContextMenu('bottom', getEventCords(e as Event, '.btn-surface'), (closeMenu: () => void) => (
      <PowerLevelSelector
        value={powerLevel}
        max={myPowerLevel}
        onSelect={(pl) => {
  ...
  return (
    <div className="room-permissions">
      {Object.keys(permsGroups).map((groupKey) => {
        const groupedPermKeys = permsGroups[groupKey as keyof typeof permsGroups]
        return (
          <div className="room-permissions__card" key={groupKey}>
            <MenuHeader>{groupKey}</MenuHeader>
            {groupedPermKeys.map((permKey) => {
              const permInfo = permissionsInfo[permKey as keyof typeof permissionsInfo]

              let powerLevel = 0
              let permValue = permInfo.parent ? permissions[permInfo.parent]?.[permKey] : permissions[permKey]
              ...
              return (
                <SettingTile
                  ...
                  options={
                    <Button onClick={canChangePermission ? (e) => handlePowerSelector(e, permKey, permInfo.parent, powerLevel) : undefined} iconSrc={canChangePermission ? ChevronBottomIC : undefined}>
                      <Text variant="b2">{`${getPowerLabel(powerLevel) || 'Member'} - ${powerLevel}`}</Text>
                    </Button>
                  }
                />
              )
