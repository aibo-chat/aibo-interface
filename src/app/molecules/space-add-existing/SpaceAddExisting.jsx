...
function SpaceAddExistingContent({ roomId }) {
  ...
  useEffect(() => {
    const allIds = [...spaces, ...rooms, ...directs].filter((rId) => rId !== roomId && !roomIdToParents.get(rId)?.has(roomId))
  ...
  const handleAdd = async () => {
    ...
    const allIds = [...spaces, ...rooms, ...directs].filter((rId) => rId !== roomId && !roomIdToParents.get(rId)?.has(roomId) && !selected.includes(rId))
    setAllRoomIds(allIds)
    setProcess(null)
    setSelected([])
  }
  ...
  return (
    <>
      ...
      {(searchIds || allRoomIds).map((rId) => {
        ...
        const parentNames = parentSet ? [...parentSet].map((parentId) => mx.getRoom(parentId).name) : undefined
        ...
        return (
          <RoomSelector
            ...
            iconSrc={directs.has(rId) ? null : joinRuleToIconSrc(room.getJoinRule(), room.isSpaceRoom())}
            isUnread={false}
            notificationCount={0}
            isAlert={false}
            onClick={handleSelect}
            options={<Checkbox isActive={selected.includes(rId)} variant="positive" onToggle={handleSelect} tabIndex={-1} disabled={process !== null} />}
          />
        )
      })}
      {selected.length !== 0 && (
