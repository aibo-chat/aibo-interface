...
function InviteList({ isOpen, onRequestClose }) {
  ...
  async function acceptInvite(roomId, isDM) {
    try {
      ...
      if (e.data?.error && typeof e.data.error === 'string') {
        procRoomError.set(roomId, e.data.error)
      } else if (typeof e.message === 'string') {
        procRoomError.set(roomId, e.message)
      } else {
        procRoomError.set(roomId, 'Something went wrong!')
      }
      updateRoomProcError(getMapCopy(procRoomError))
    }
  }
  ...
  function renderRoomTile(roomId) {
    ...
    const roomConditions = myRoom?.currentState.getStateEvents('m.room.condition')[0]?.getContent().room_conditions
    const renderError = () => {
      if (!procRoomError.has(roomId)) return null
      return (
        <Text variant="b2">
          <span style={{ color: 'var(--bg-danger)' }}>{procRoomError.get(roomId)}</span>
        </Text>
      )
    }
    return (
  ...
  return (
    <PopupWindow isOpen={isOpen} title="Invites" contentOptions={<IconButton src={CrossIC} onClick={onRequestClose} tooltip="Close" />} onRequestClose={onRequestClose}>
