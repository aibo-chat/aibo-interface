...
function InviteUser({ isOpen, roomId, searchTerm, onRequestClose }) {
  ...
  async function searchUser(username) {
    ...
    if (inputUsername === '') {
      updateSearchQuery({ username: undefined })
      updateUsers([])
      return
    }
  ...
  async function createDM(userId, openAfterCreate = false) {
    ...
    try {
      ...
      if (openAfterCreate && result?.room_id) {
  ...
  async function inviteToRoom(userId) {
    ...
    try {
      ...
      if (e.data?.error && typeof e.data.error === 'string') {
        procUserError.set(userId, e.data.error)
      } else if (typeof e.message === 'string') {
        procUserError.set(userId, e.message)
      } else {
        procUserError.set(userId, 'Something went wrong!')
      }
      updateUserProcError(getMapCopy(procUserError))
    }
  }
  function renderUserList(users, isRecommendedUser = false) {
    const renderOptions = (userId) => {
      const messageJSX = (message, isPositive) => (
        <Text variant="b2">
          <span style={{ color: isPositive ? 'var(--bg-positive)' : 'var(--bg-negative)' }}>{message}</span>
        </Text>
      )
      ...
      return typeof roomId === 'string' ? (
        ...
        <Button onClick={() => createDM(userId, isRecommendedUser)} variant="primary">
          Message
        </Button>
      )
    }
    ...
    return users.map((user) => {
      ...
      const avatarSrc = typeof user.avatar_url === 'string' ? mx.mxcUrlToHttp(user.avatar_url, 42, 42, 'crop') : user.defed_avatar_url || null
      return <RoomTile key={userId} avatarSrc={avatarSrc} name={name} id={userId} options={renderOptions(userId)} desc={renderError(userId)} />
    })
  }

  const renderRecommendUserList = () => {
    const recommendUsers = notMigratedRelationship.map((conv) => ({
      defed_avatar_url: conv.avatarLink,
      display_name: conv.handleName || conv.name || conv.toProxy,
      user_id: `@${conv.toProxy}:${MatrixHomeServer}`,
    }))
    return (
      <>
        <div className="invite-user__search-status">
          <Text variant="b2">Recommended Contacts</Text>
        </div>
        <div className="invite-user__content">{renderUserList(recommendUsers, true)}</div>
      </>
    )
  }
  ...
  const renderListPart = () => {
    if (users.length !== 0) {
      return <div className="invite-user__content">{renderUserList(users)}</div>
    }
    if ((!searchQuery?.username || !users?.length) && notMigratedRelationship.length) {
      return renderRecommendUserList()
    }
    return null
  }
  useEffect(() => {
    ...
    >
      <div className="invite-user">
        ...
        <div className="invite-user__search-status">
          ...
          {typeof searchQuery.username !== 'undefined' && !isSearching && <Text variant="b2">{`Search result for user "${searchQuery.username}"`}</Text>}
          {searchQuery.error && (
