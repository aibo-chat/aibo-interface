...
function Drawer({ drawerPostie }) {
  const {
    renderStore: { _ROOM_LIST_UPDATED, updateRoomList },
  ...
  return (
    <Box
      className="drawer"
      sx={{
        width: iframeAppData && iframeAppDisplay ? 0 : 'auto',
        flex: iframeAppData && iframeAppDisplay ? 0 : 1,
      }}
    >
      <DrawerHeader selectedTab={selectedTab} spaceId={spaceId} />
      <div className="drawer__content-wrapper">
        {navigation.selectedSpacePath.length > 1 && selectedTab !== cons.tabs.DIRECTS && <DrawerBreadcrumb spaceId={spaceId} />}
        <div className="rooms__wrapper">
          <ScrollView ref={scrollRef} autoHide>
            <div className="rooms-container">{selectedTab !== cons.tabs.DIRECTS ? <Home spaceId={spaceId} drawerPostie={drawerPostie} /> : <Directs size={roomList.directs.size} />}</div>
          </ScrollView>
        </div>
      </div>
      {systemState !== null && (
