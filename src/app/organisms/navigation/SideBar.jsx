...
import { selectTab, openShortcutSpaces, openInviteList, openSearch, openSettings, openReusableContextMenu } from '../../../client/action/navigation'
...
import SettingIC from '../../../../public/res/svg/common/common_outlined_setting_icon.svg'
...
import SideBarAIBotPart from './SideBarAIBotPart'
import SideBarAppPart from './SideBarAppPart'
...
const ProfileAvatarMenu = observer(() => {
  ...
  return <SidebarAvatar onClick={onButtonClick} tooltip="Settings" avatar={<Avatar iconSrc={SettingIC} bgColor={colorMXID(mx.getUserId())} size="normal" />} />
})
  ...
  return (
    <>
      <SidebarAvatar
        ...
        avatar={
          <Avatar text={profile.displayName} bgColor={colorMXID(mx.getUserId())} size="normal" imageSrc={profile.avatarUrl !== null ? mx.mxcUrlToHttp(profile.avatarUrl, 42, 42, 'crop') : null} />
        }
        notificationBadge={homeNoti ? <NotificationBadge alert={homeNoti?.highlight > 0} content={abbreviateNumber(homeNoti.total) || null} /> : null}
      />
      {developerMode ? (
        <SidebarAvatar
          tooltip="People"
          ...
          notificationBadge={dmsNoti ? <NotificationBadge alert={dmsNoti?.highlight > 0} content={abbreviateNumber(dmsNoti.total) || null} /> : null}
        />
      ) : null}
    </>
  )
...
function DraggableSpaceShortcut({ isActive, spaceId, index, moveShortcut, onDrop }) {
  ...
  const openSpaceOptions = (e, sId) => {
    e.preventDefault()
    openReusableContextMenu('right', getEventCords(e, '.sidebar-avatar'), (closeMenu) => <SpaceOptions roomId={sId} afterOptionSelect={closeMenu} />)
  }
  ...
  return (
    <SidebarAvatar
      ...
      avatar={<Avatar ref={avatarRef} text={room.name} bgColor={colorMXID(room.roomId)} size="normal" imageSrc={room.getAvatarUrl(initMatrix.matrixClient.baseUrl, 42, 42, 'crop') || null} />}
      notificationBadge={
        notifications.hasNoti(spaceId) ? <NotificationBadge alert={notifications.getHighlightNoti(spaceId) > 0} content={abbreviateNumber(notifications.getTotalNoti(spaceId)) || null} /> : null
      }
    />
  )
}
  ...
  return (
    <DndProvider backend={HTML5Backend}>
      {spaceShortcut.map((shortcut, index) => (
        <DraggableSpaceShortcut key={shortcut} index={index} spaceId={shortcut} isActive={selectedTab === shortcut} moveShortcut={moveShortcut} onDrop={handleDrop} />
      ))}
    </DndProvider>
  )
}
...
function useTotalInvites() {
  ...
  const totalInviteCount = () => roomList.inviteRooms.size + roomList.inviteSpaces.size + roomList.inviteDirects.size
      ...
      <div className="sidebar__scrollable">
        <ScrollView invisible>
          <div className="scrollable-content">
            ...
            <SideBarAIBotPart drawerPostie={drawerPostie} />
