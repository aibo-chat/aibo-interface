...
import { getUsername, getUsernameOfRoomMember, getPowerLabel, hasDMWith, hasDevices } from '../../../util/matrixUtil'
import { getEventCords, substitutionString, substitutionStringForMatrixId } from '../../../util/common'
...
function ModerationTools({ roomId, userId }) {
  ...
  const canIKick = roomMember?.membership === 'join' && room.currentState.hasSufficientPowerLevelFor('kick', myPowerLevel) && powerLevel < myPowerLevel
  const canIBan = ['join', 'leave'].includes(roomMember?.membership) && room.currentState.hasSufficientPowerLevelFor('ban', myPowerLevel) && powerLevel < myPowerLevel
...
function SessionInfo({ userId }) {
  ...
  function renderSessionChips() {
    ...
    return (
      <div className="session-info__chips">
        ...
        {devices !== null && devices.map((device) => <Chip key={device.deviceId} iconSrc={ShieldEmptyIC} text={device.getDisplayName() || device.deviceId} />)}
      </div>
    )
  }
  ...
  return (
    <div className="session-info">
      <MenuItem onClick={() => setIsVisible(!isVisible)} iconSrc={isVisible ? ChevronBottomIC : ChevronRightIC}>
    ...
    <div className="profile-viewer__buttons">
      ...
      {(isInvited ? canIKick : room.canInvite(mx.getUserId())) && isInvitable && (
        <Button onClick={toggleInvite} disabled={isInviting}>
          {isInvited ? `${isInviting ? 'Disinviting...' : 'Disinvite'}` : `${isInviting ? 'Inviting...' : 'Invite'}`}
        </Button>
      )}
      <Button variant={isUserIgnored ? 'positive' : 'danger'} onClick={toggleIgnore} disabled={isIgnoring}>
        {isUserIgnored ? `${isIgnoring ? 'Unignoring...' : 'Unignore'}` : `${isIgnoring ? 'Ignoring...' : 'Ignore'}`}
      </Button>
    </div>
  )
}
...
function useRerenderOnProfileChange(roomId, userId) {
  ...
  useEffect(() => {
    const handleProfileChange = (mEvent, member) => {
      if (mEvent.getRoomId() === roomId && (member.userId === userId || member.userId === mx.getUserId())) {
        forceUpdate()
      }
    }
...
const ProfileViewer = () => {
  ...
  const isDM = roomIdToProxyMap.has(roomId)
  ...
  const renderProfile = () => {
    ...
    const canChangeRole = room.currentState.maySendEvent('m.room.power_levels', mx.getUserId()) && (powerLevel < myPowerLevel || userId === mx.getUserId())
    ...
    const handleChangePowerLevel = async (newPowerLevel) => {
      ...
      if (isSharedPower || isDemotingMyself) {
        const isConfirmed = await confirmDialog('Change power level', isSharedPower ? SHARED_POWER_MSG : DEMOTING_MYSELF_MSG, 'Change', 'caution')
    ...
    return (
      <div className="profile-viewer">
        <div className="profile-viewer__user">
          ...
          <div className="profile-viewer__user__info">
            <Text variant="s1" weight="medium">
              {twemojify(substitutionString(username))}
            </Text>
            <br />
            <Text variant="b2">{twemojify(substitutionStringForMatrixId(userId))}</Text>
          </div>
          {!isDM ? (
            <div className="profile-viewer__user__role">
              <Text variant="b3">Role</Text>
              <Button onClick={canChangeRole ? handlePowerSelector : null} iconSrc={canChangeRole ? ChevronBottomIC : null}>
        ...
        {developerMode ? <SessionInfo userId={userId} /> : null}
        {userId !== mx.getUserId() && !isDM && <ProfileFooter roomId={roomId} userId={userId} onRequestClose={closeDialog} />}
      </div>
    )
  }
...
export default observer(ProfileViewer)
