...
function SpaceManageItem({ parentId, roomInfo, onSpaceClick, requestClose, isSelected, onSelect, roomHierarchy }) {
  ...
  const handleJoin = async () => {
    try {
      ...
      await join(roomId, false, via)
    } catch (e) {
      setIsJoining(false)
      if (e?.data?.error) {
        snackbarUtils.error(e.data.error)
      } else if (e?.message) {
  ...
  const expandBtnJsx = <IconButton variant={isExpand ? 'primary' : 'surface'} size="extra-small" src={InfoIC} tooltip="Topic" tooltipPlacement="top" onClick={() => setIsExpand(!isExpand)} />
  ...
  return (
    <div className={`space-manage-item${isSpace ? '--space' : ''}`}>
      <div>
        ...
        <button className="space-manage-item__btn" onClick={isSpace ? () => onSpaceClick(roomId, name) : null} type="button">
          {roomAvatarJSX}
          {roomNameJSX}
          {roomInfo?.room_conditions?.length ? (
            <CommonConditionDisplay
              roomConditions={roomInfo.room_conditions}
              sx={{
                marginRight: '8px',
              }}
            />
        ...
        {isJoined ? (
          <Button style={{ flexShrink: 0 }} onClick={handleOpen}>
            Open
          </Button>
        ) : (
          <Button style={{ flexShrink: 0 }} variant="primary" onClick={handleJoin} disabled={isJoining}>
    ...
    <div className="space-manage__footer">
      ...
      {!process && (
        <>
          ...
          <Button onClick={() => handleToggleSuggested(!allSuggested)} variant={allSuggested ? 'surface' : 'primary'}>
  ...
  return (
    <div className="space-manage__content">
      ...
      <div className="space-manage__content-items">
        {!isLoading && currentHierarchy?.rooms?.length === 1 && <Text>Either the space contains private rooms or you need to join space to view it's rooms.</Text>}
        {currentHierarchy &&
      ...
      {currentHierarchy?.canLoadMore && !isLoading && <Button onClick={loadRoomHierarchy}>Load more</Button>}
      {isLoading && (
      ...
      {selected.length > 0 && <SpaceManageFooter parentId={currentPath.roomId} selected={selected} />}
    </div>
  )
}
