...
import { EventTimeline, EventTimelineSet, EventType, IPushRule, IPushRules, JoinRule, MatrixClient, MatrixEvent, MsgType, NotificationCountType, RelationType, Room, RoomMember } from 'matrix-js-sdk'
import { CryptoBackend } from 'matrix-js-sdk/lib/common-crypto/CryptoBackend'
import { AccountDataEvent } from '../../types/matrix/accountData'
import { MessageEvent, NotificationType, RoomToParents, RoomType, StateEvent, UnreadInfo } from '../../types/matrix/room'
import { DefedMsgType } from '../../types/defed/message'
import { FeedsNewsMessageContent } from '../components/message/FeedsNewsMessage'

export const getStateEvent = (room: Room, eventType: StateEvent, stateKey = ''): MatrixEvent | undefined => room.currentState.getStateEvents(eventType, stateKey) ?? undefined

export const getStateEvents = (room: Room, eventType: StateEvent): MatrixEvent[] => room.currentState.getStateEvents(eventType)

export const getAccountData = (mx: MatrixClient, eventType: AccountDataEvent): MatrixEvent | undefined => mx.getAccountData(eventType)
...
export const mapParentWithChildren = (roomToParents: RoomToParents, roomId: string, children: string[]) => {
...
export const isMutedRule = (rule: IPushRule) => rule.actions[0] === 'dont_notify' && rule.conditions?.[0]?.kind === 'event_match'

export const findMutedRule = (overrideRules: IPushRule[], roomId: string) => overrideRules.find((rule) => rule.rule_id === roomId && isMutedRule(rule))
...
export const getNotificationType = (mx: MatrixClient, roomId: string): NotificationType => {
  ...
  if (!roomPushRule) {
    const overrideRules = mx.getAccountData('m.push_rules')?.getContent<IPushRules>()?.global?.override
...
export const isNotificationEvent = (mEvent: MatrixEvent) => {
  ...
  if (['m.room.create', 'm.room.message', 'm.room.encrypted', 'm.room.member', 'm.sticker'].find((type) => type === eType)) return false
...
export const joinRuleToIconSrc = (icons: Record<IconName, IconSrc>, joinRule: JoinRule, space: boolean): IconSrc | undefined => {
...
export const getRoomAvatarUrl = (mx: MatrixClient, room: Room): string | undefined => {
  const url = room.getAvatarFallbackMember()?.getAvatarUrl(mx.baseUrl, 32, 32, 'crop', undefined, false) ?? undefined
  if (url) return url
...
export const parseReplyBody = (userId: string, body: string) => `> <${userId}> ${body.replace(/\n/g, '\n> ')}\n\n`

export const parseReplyFormattedBody = (roomId: string, userId: string, eventId: string, formattedBody: string): string => {
  const replyToLink = `<a href="https://matrix.to/#/${encodeURIComponent(roomId)}/${encodeURIComponent(eventId)}">In reply to</a>`
...
export const getMemberSearchStr = (member: RoomMember, query: string, mxIdToName: (mxId: string) => string): string[] => [
...
export const isMembershipChanged = (mEvent: MatrixEvent): boolean =>
  mEvent.getContent().membership !== mEvent.getPrevContent().membership || mEvent.getContent().reason !== mEvent.getPrevContent().reason
...
export const getEventReactions = (timelineSet: EventTimelineSet, eventId: string) => timelineSet.relations.getChildEventsForEvent(eventId, RelationType.Annotation, EventType.Reaction)

export const getEventEdits = (timelineSet: EventTimelineSet, eventId: string, eventType: string) => timelineSet.relations.getChildEventsForEvent(eventId, RelationType.Replace, eventType)

export const getLatestEdit = (targetEvent: MatrixEvent, editEvents: MatrixEvent[]): MatrixEvent | undefined => {
  const eventByTargetSender = (rEvent: MatrixEvent) => rEvent.getSender() === targetEvent.getSender()
  return editEvents.sort((m1, m2) => m2.getTs() - m1.getTs()).find(eventByTargetSender)
}

export const getEditedEvent = (mEventId: string, mEvent: MatrixEvent, timelineSet: EventTimelineSet): MatrixEvent | undefined => {
...
export const canEditEvent = (mx: MatrixClient, mEvent: MatrixEvent) =>
  ...
  (mEvent.getContent().msgtype === MsgType.Text || mEvent.getContent().msgtype === MsgType.Emote || mEvent.getContent().msgtype === MsgType.Notice)

export const getLatestEditableEvt = (timeline: EventTimeline, canEdit: (mEvent: MatrixEvent) => boolean): MatrixEvent | undefined => {
...
export const reactionOrEditEvent = (mEvent: MatrixEvent) => mEvent.getRelation()?.rel_type === RelationType.Annotation || mEvent.getRelation()?.rel_type === RelationType.Replace
