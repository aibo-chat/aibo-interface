...
class RoomList extends EventEmitter {
  ...
  getOrphansDirects() {
    return [...this.directs].filter((roomId) => true)
  }
  ...
  getRoomType(room) {
    if (this.mDirects.has(room.roomId)) return 'direct'
  }
  ...
  _populateRooms() {
    ...
    this.matrixClient.getRooms().forEach((room) => {
      ...
      if (room.getMyMembership() === 'invite') {
        if (this._isDMInvite(room)) {
          this.inviteDirects.add(roomId)
        } else if (room.isSpaceRoom()) {
          this.inviteSpaces.add(roomId)
        } else {
          this.inviteRooms.add(roomId)
        }
        return
      }
  ...
  _listenEvents() {
    ...
    this.matrixClient.on('RoomState.events', (mEvent, state) => {
      ...
      if (['m.room.avatar', 'm.room.topic', 'm.room.condition'].includes(mEvent.getType())) {
        if (['m.room.avatar', 'm.room.condition'].includes(mEvent.getType())) {
          this.emit(cons.events.roomList.ROOMLIST_UPDATED)
        }
