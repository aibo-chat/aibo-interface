...
class Navigation extends EventEmitter {
  ...
  _mapRoomToSpace(roomId) {
    ...
    if (this.selectedTab === cons.tabs.HOME && roomList.rooms.has(roomId) && !roomList.roomIdToParents.has(roomId)) {
  ...
  _selectRoom(roomId, eventId) {
    ...
    this.emit(cons.events.navigation.ROOM_SELECTED, this.selectedRoomId, prevSelectedRoomId, eventId)
  }
  ...
  _selectRoomWithTab(tabId) {
    ...
    if (tabId === cons.tabs.DIRECTS) {
      const data = this.spaceToRoom.get(tabId)
      if (data) {
        this._selectRoom(data.roomId)
        return
      }
      const children = [...roomList.directs]
      this._selectRoom(this._getLatestActiveRoomId(children))
      return
    }
  ...
  navigate(action) {
    const actions = {
      [cons.actions.navigation.SELECT_TAB]: () => {
        const roomId = action.tabId !== cons.tabs.HOME && action.tabId !== cons.tabs.DIRECTS ? action.tabId : null
      ...
      [cons.actions.navigation.TOGGLE_ROOM_SETTINGS]: () => {
        this.isRoomSettings = !this.isRoomSettings
        this.emit(cons.events.navigation.ROOM_SETTINGS_TOGGLED, this.isRoomSettings, action.tabText)
      },
      ...
      [cons.actions.navigation.OPEN_EMOJIBOARD]: () => {
        this.emit(cons.events.navigation.EMOJIBOARD_OPENED, action.cords, action.requestEmojiCallback)
      },
      ...
      [cons.actions.navigation.CLICK_REPLY_TO]: () => {
        this.emit(cons.events.navigation.REPLY_TO_CLICKED, action.userId, action.eventId, action.body, action.formattedBody)
      },
      ...
      [cons.actions.navigation.OPEN_REUSABLE_CONTEXT_MENU]: () => {
        this.emit(cons.events.navigation.REUSABLE_CONTEXT_MENU_OPENED, action.placement, action.cords, action.render, action.afterClose)
      },
      [cons.actions.navigation.OPEN_REUSABLE_DIALOG]: () => {
        this.emit(cons.events.navigation.REUSABLE_DIALOG_OPENED, action.title, action.render, action.afterClose)
      },
      [cons.actions.navigation.OPEN_EMOJI_VERIFICATION]: () => {
        this.emit(cons.events.navigation.EMOJI_VERIFICATION_OPENED, action.request, action.targetDevice)
      },
    }
    actions[action.type]?.()
  }
