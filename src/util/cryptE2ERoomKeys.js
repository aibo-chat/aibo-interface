...
async function deriveKeys(salt, iterations, password) {
  ...
  let key
  try {
    key = await subtleCrypto.importKey('raw', new TextEncoder().encode(password), { name: 'PBKDF2' }, false, ['deriveBits'])
  } catch (e) {
  ...
  const aesProm = subtleCrypto.importKey('raw', aesKey, { name: 'AES-CTR' }, false, ['encrypt', 'decrypt']).catch((e) => {
